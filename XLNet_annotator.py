...
flags.DEFINE_string("input_file", None, "File to load to be processed.")
flags.DEFINE_string("output_file", "xlnet_output", "Name of file to save predictions to")
...
flags.DEFINE_float("lr_layer_decay_rate", .75, "Top layer: lr[L] = FLAGS.learning_rate. Low layer: lr[l-1] = lr[l] * lr_layer_decay_rate.")
...
class NerProcessor(object):
    ...
    def get_chem_examples(self):
        """Gets chemical training data."""
        data_path = os.path.join(self.data_dir, self.input_file) 
        data_list = self._read_data(data_path)
        example_list = self._create_chem_example(data_list)
        return example_list
    ...
    def get_test_examples(self):
        ...
        data_list = self._read_citation_data(self.data_dir)
        example_list = self._create_chem_example(data_list)
        return example_list
    
    def get_labels(self):
        """Gets the list of labels for this data set."""
        # CoNLL labels: <pad> O X <cls> <sep> B-LOC B-MISC B-ORG B-PER I-LOC I-MISC I-ORG I-PER
        label_list = ["<pad>", "O", "X", "<cls>", "<sep>", "B-chem", "I-chem"]
        return label_list
    ...
    def _read_citation_data(cls, citation_dir):
        """
    ...
    def _create_chem_example(self, lines):
        examples = []
...
class XLNetTokenizer(object):
    ...
    def tokenize(self,
        ...
        # Changed to unicode=True, 7-10-2019
    ...
    def get_model_fn(self,
        ...
        def model_fn(features,
            ...
            loss, predict_ids = self._create_model(model_config, run_config,
                input_ids, input_masks, segment_ids, label_ids, label_list, mode)
...
class XLNetPredictRecorder(object):
    ...
    def _write_output(self, tokens, predictions, guid):
        """
        Write pmid, tokens, and predictions for each token to file
        """
        with open("{}/results/decoded_tokens.txt".format(FLAGS.output_dir), "a", encoding="utf8") as f:
            for token, pred in zip(tokens, predictions):
                token = token.strip(prepro_utils.SPIECE_UNDERLINE)
                f.write("{0}\t{1}\t{2}\n".format(guid, token, pred))
    ...
    def record(self,
        ...
        decoded_tokens_file = open("{}/results/decoded_tokens.txt".format(FLAGS.output_dir), "w", encoding="utf8")
        ...
        for predict, guid in zip(predicts, self.guids):
            ...
            for input_token, input_mask, output_predict in results:
                ...
                results_file.write("{0}\t{1}\t{2}\n".format(guid, input_token, output_predict))
...
def main(_):
    ...
    model_fn = model_builder.get_model_fn(model_config, run_config, FLAGS.init_checkpoint, label_list)
